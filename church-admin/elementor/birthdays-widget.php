...
class Elementor_church_admin_birthdays_widget extends \Elementor\Widget_Base {

	public function get_name() {
		return 'churchAdminBirthdays';
	}

	public function get_title() {
		return esc_html(__('Birthdays','church-admin'));
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'birthdays',
			[
				'label' => esc_html__( 'Birthdays Options', 'church-admin' ),
		...
		$this->add_control(
			'show_age',
			[
				'label' => esc_html__( 'Show age', 'church-admin' ),
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Yes', 'church-admin' ),
				'label_off' => esc_html__( 'No', 'church-admin' ),
				'return_value' => 1,
	...
	protected function render() {
		...
		if(!empty($settings['logged_in']) && !is_user_logged_in()){
            ...
            if ( empty( $attributes['loggedin'] )||is_user_logged_in() )
            {
                ...
                echo church_admin_frontend_birthdays( (int)$membts,(int)$peoplets, (int)$settings['days'],(int)$settings['show_age'],(int)$settings['show_email'],(int)$settings['show_phone'] );
            }
            else //login required
            {
